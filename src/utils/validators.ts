function validateCpf(cpf: string) {
  ...
  let plus = 0
  let rest
  for (let i = 1; i <= 9; i++) plus = plus + parseInt(cpf.substring(i - 1, i)) * (11 - i)
  rest = (plus * 10) % 11
  if (rest == 10 || rest == 11) rest = 0
  if (rest != parseInt(cpf.substring(9, 10))) return false
  plus = 0
  for (let i = 1; i <= 10; i++) plus = plus + parseInt(cpf.substring(i - 1, i)) * (12 - i)
  rest = (plus * 10) % 11
  if (rest == 10 || rest == 11) rest = 0
...
function validateName(name: string) {
  const nameOnlyLetters = onlyLetters(name)
...
function onlyNumbers(phrase: string) {
  const removedNumbers = phrase.replace(/\D/g, '')
  const removedSpecialCharacters = removedNumbers.replace(/[&/\\#,+()$~%.'":*?<>{}[\]@!]/g, '')

  return removedSpecialCharacters
}

function onlyLetters(phrase: string) {
  const removedNumbers = phrase.replace(/[\d]/g, '')
  const removedSpecialCharacters = removedNumbers.replace(/[&/\\#,+()$~%.'":*?<>{}[\]@!]/g, '')
  return removedSpecialCharacters
}

function onlyLettersAndNumbers(phrase: string) {
  const removedSpecialCharacters = phrase.replace(/[&/\\#,+()$~%.'":*?<>{}[\]@!-]/g, '')
  return removedSpecialCharacters
}

export { onlyLetters, onlyLettersAndNumbers, onlyNumbers, validateCpf, validateDate, validateName, validatePassport }
